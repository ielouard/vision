...
def track(image,color):
    ...
    global radius 
    delta = 50
    if  not Object.size==0:
      if  not radius==0:
        r = cv2.boundingRect(Object)
        height, width, channels = image.shape
        imgtoana = np.zeros((height,width,3), np.uint8)
        r=list(r)
        if (r[1]+r[3])<height:
         r[3]=r[3]+delta
        if (r[0]+r[2])<width:
        ...
        if r[2]>20:
         r[0]=r[0]-delta
        imgtoana[r[1]:r[1]+r[3], r[0]:r[0]+r[2]] = image[r[1]:r[1]+r[3], r[0]:r[0]+r[2]]
        cv2.imshow("Image", imgtoana)
    else:
     imgtoana=image
    screenMidX = 1260/2
    screenMidY = 960/2
    ...
    blur = cv2.GaussianBlur(imgtoana, (5,5),0)

    # Convert BGR to HSV
    hsv = cv2.cvtColor(blur, cv2.COLOR_BGR2HSV)
    frame = imgtoana[:, :, ::-1].copy()
